...
extension DatabaseManager {
    ...
    public func createNewConversation(with otherUserEmail: String, name: String, firstMessage: Message, completion: @escaping (Bool) -> Void) {
        guard let currentEmail = UserDefaults.standard.value(forKey: Keys.email.rawValue) as? String,
              let currentName = UserDefaults.standard.value(forKey: Keys.name.rawValue) as? String else {
                  return
              }
        ...
        reference.observeSingleEvent(of: .value) { [weak self] snapshot in
            ...
            if var conversations = userNode["conversations"] as? [[String: Any]] {
                ...
                reference.setValue(userNode) { [weak self] error, _ in
                    ...
                    self?.finishCreatedConversation(name: name,
    ...
    private func finishCreatedConversation(name: String, conversationId: String, firstMessage: Message, completion: @escaping (Bool) -> Void) {
                ...
                strongSelf.database.child("\(currentEmail)/conversations").observeSingleEvent(of: .value) { snapshot in
                    ...
                    strongSelf.database.child("\(currentEmail)/conversations").setValue(databaseEntryConversations) { error, _ in
                        ...
                        strongSelf.database.child("\(otherUserEmail)/conversations").observeSingleEvent(of: .value) { snapshot in
                            ...
                            guard let currentName = UserDefaults.standard.value(forKey: Keys.name.rawValue) as? String else {
                                return
                            }
    ...
    public func deleteConversation(conversationId: String, completion: @escaping (Bool) -> Void) {
        ...
        // Delete conversation in collection with targer id
        // reset those conversations for the user in database
        let ref = database.child("\(safeEmail)/conversations")
        ref.observeSingleEvent(of: .value) { snapshot in
            if var conversatios = snapshot.value as? [[String: Any]] {
                var positionToRemove = 0
                for conversation in conversatios {
                ...
                conversatios.remove(at: positionToRemove)
                ref.setValue(conversatios) { error, _ in
                    guard error == nil else {
                        completion(false)
    ...
    public func conversationExists(with targetRecipientEmail: String, completion: @escaping (Result<String, Error>) -> Void) {
        ...
        guard let senderEmail = UserDefaults.standard.value(forKey: Keys.email.rawValue) as? String else {
            return
        }
